...
namespace Yamashita.MultiTracker
{
    public class MultiTracker : IMultiTracker
    {
        ...
        public void Update(ref Mat frame, List<(string Label, float Iou, Rect2d Box)> detections, out List<(int Id, string Label, float Iou, Rect2d Box)> results)
        {
        ...
        private void Assign(List<(string Label, float Iou, Rect2d Box)> detections)
        {
            foreach (var detection in detections)
            {
                ...
                if (first)
                {
                    _trackers.Add(new Individual(detection.Box, detection.Iou, _id++, detection.Label));
                }
            }
        }
